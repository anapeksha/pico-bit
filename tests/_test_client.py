...
from micro_server import AsyncBytesIO, Request, Response

try:
    from micro_server import WebSocket  # type: ignore[attr-defined]
        ...
        class FakeWebSocket:
            ...
            async def _next(self, data=None):
                try:
                    ...
                    if not self.closed:
                        self.closed = True
                        raise OSError(32, 'Websocket connection closed') from None
            ...
            async def awrite(self, data):
                if self.started:
                    ...
                    if h[1] not in [WebSocket.TEXT, WebSocket.BINARY]:
                        return
                    data = data[2 - h[3]:] if h[3] < 0 else data[2:]
