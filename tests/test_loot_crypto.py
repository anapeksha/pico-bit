...
def test_encrypt_produces_magic_header() -> None:
    ...
    data = asyncio.run(encrypt('{"hello": "world"}', key))
...
def test_encrypt_decrypt_roundtrip() -> None:
    ...
    encrypted = asyncio.run(encrypt(plaintext, key))
    recovered = asyncio.run(decrypt(encrypted, key))
...
def test_decrypt_roundtrip_with_unicode() -> None:
    ...
    encrypted = asyncio.run(encrypt(text, key))
    assert json.loads(asyncio.run(decrypt(encrypted, key)))['msg'] == 'héllo wörld 🔑'
...
def test_decrypt_backward_compat_with_plaintext_file() -> None:
    ...
    assert asyncio.run(decrypt(raw, key)) == plaintext
...
def test_different_nonces_produce_different_ciphertext() -> None:
    ...
    c1 = asyncio.run(encrypt(text, key))
    c2 = asyncio.run(encrypt(text, key))
    # Both decrypt correctly
    assert asyncio.run(decrypt(c1, key)) == text
    assert asyncio.run(decrypt(c2, key)) == text
