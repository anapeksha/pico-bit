<script lang="ts">
  ...
  import { MAX_ARMORY_FILE_SIZE, formatBytes, validateArmoryFile } from '../lib/binary';
  import {
    armoryNotice,
    armoryFiles,
    deleteBinary,
  ...
  async function selectFile(file: File) {
    const error = await validateArmoryFile(file, MAX_ARMORY_FILE_SIZE);
    if (error) {
      selectedFile = null;
      fileError = error;
  ...
  async function uploadSelected() {
    ...
    const error = await validateArmoryFile(selectedFile, MAX_ARMORY_FILE_SIZE);
    if (error) {
      fileError = error;
      return;
    }
...
<section class="flex min-h-0 shrink-0 flex-col">
  ...
  <div class="flex min-h-0 flex-1 flex-col overflow-y-auto">
    <section class="flex flex-col min-h-0 flex-1" aria-labelledby="ducky-editor-heading">
      <div
        class="flex items-center gap-2.5 border-b border-(--border) bg-(--surface-3) px-3.5 py-2"
      >
        <h2
          id="ducky-editor-heading"
          class="m-0 font-mono text-[11px] font-bold text-(--text) uppercase tracking-wider"
        >
          Ducky Script Editor
        </h2>
      </div>
    ...
    <section aria-labelledby="binary-armory-heading">
      <div
        class="border-t border-(--border) bg-(--surface-2) px-3.5 py-2 flex items-center gap-2.5"
      >
        <h2
          id="binary-armory-heading"
          class="m-0 font-mono text-[11px] font-bold text-picobit-text uppercase tracking-wider"
        >
          Binary Armory
        </h2>
      </div>
