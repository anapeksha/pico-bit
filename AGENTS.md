...
* **CI Cargo Caching:** GitHub Actions should use `cargo-chef prepare` plus a recipe-keyed `actions/cache` entry before `cargo chef cook`. This keeps dependency builds cached when `Cargo.toml`/`Cargo.lock` have not changed while still rebuilding firmware source normally.
